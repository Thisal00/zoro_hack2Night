    ...
    <div className="flex h-screen w-full bg-slate-50 font-sans text-slate-900 overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto px-6 py-8 md:px-10 lg:px-12">
        <header className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-slate-900">Bank Transfer</h1>
            <p className="text-sm text-slate-500 mt-1">Send funds securely to any account.</p>
          </div>
                ...
                <div className="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden z-50">
                  ...
                  <div className="p-2 space-y-1">
                    <div className="p-3 rounded-xl hover:bg-slate-50 transition-colors cursor-pointer">
                      <p className="text-sm font-medium text-slate-900">Scheduled Transfer Completed</p>
                      <p className="text-xs text-slate-500 mt-1">Rent payment sent successfully.</p>
                    </div>
                  </div>
                </div>
              )}
          ...
          <div className="rounded-3xl border border-slate-200 bg-white shadow-sm p-8 relative overflow-hidden">
            ...
            {step === 'form' && (
              <form onSubmit={handleNext} className="relative z-10 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-12 gap-y-6 md:gap-x-8 items-center">
                  <label className="md:col-span-3 text-sm font-medium text-slate-700">From Account</label>
                  ...
                  <label className="md:col-span-3 text-sm font-medium text-slate-700">Amount (Rs.)</label>
                  <div className="md:col-span-9">
                    <input
                      type="number"
                      value={amount}
                  ...
                  <label className="md:col-span-3 text-sm font-medium text-slate-700">Account Number</label>
                  <div className="md:col-span-9">
                    <input
                      type="text"
                      value={accountNumber}
                    ...
                    />
                    {errors.accountNumber && <p className="text-xs text-red-500 mt-1.5">{errors.accountNumber}</p>}
                  </div>

                  <label className="md:col-span-3 text-sm font-medium text-slate-700">Account Name</label>
                  <div className="md:col-span-9">
                    <input
                      type="text"
                      value={accountName}
                    ...
                    />
                    {errors.accountName && <p className="text-xs text-red-500 mt-1.5">{errors.accountName}</p>}
                  </div>

                  <label className="md:col-span-3 text-sm font-medium text-slate-700">Select Bank</label>
                  <div className="md:col-span-9">
                    <select
                      value={bank}
                    ...
                    {errors.bank && <p className="text-xs text-red-500 mt-1.5">{errors.bank}</p>}
                  </div>

                  <label className="md:col-span-3 text-sm font-medium text-slate-700">Description</label>
                  <div className="md:col-span-9">
                    <textarea
                      value={description}
                ...
                <div className="flex justify-end mt-8 pt-6 border-t border-slate-100">
                  <button type="submit" className="rounded-xl bg-[#e65a28] px-8 py-3.5 font-bold text-white shadow-md shadow-orange-500/20 transition-all hover:bg-[#d44d1e] hover:shadow-lg hover:shadow-orange-500/30">
                    Continue to Review
                  </button>
                </div>
              </form>
            ...
            {step === 'confirm' && (
              <div className="relative z-10 text-center py-6">
                <h3 className="text-2xl font-bold text-slate-900 mb-8">Review Transfer</h3>
                ...
                <div className="rounded-2xl bg-slate-50 border border-slate-200 p-6 mb-8 max-w-md mx-auto text-left">
                  <div className="flex justify-between border-b border-slate-200 pb-4 mb-4">
                    ...
                    <span className="font-semibold text-slate-900 text-right">{accounts.find(a => a.account_number === fromAccount)?.account_name || 'My Account'}<br/><span className="text-sm font-normal text-slate-500">{fromAccount}</span></span>
                  </div>
                  <div className="flex justify-between border-b border-slate-200 pb-4 mb-4">
                    <span className="text-slate-500">Recipient</span>
                    <span className="font-semibold text-slate-900 text-right">{accountName}<br/><span className="text-sm font-normal text-slate-500">{accountNumber} • {bank}</span></span>
                  </div>
                  <div className="flex justify-between border-b border-slate-200 pb-4 mb-4">
                    <span className="text-slate-500">Transfer Amount</span>
                    <span className="font-semibold text-slate-900">Rs. {Number(amount).toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-200 pb-4 mb-4">
                    <span className="text-slate-500">Transfer Fee</span>
                    <span className="font-semibold text-slate-900">Rs. 50.00</span>
                  </div>
                  <div className="flex justify-between pt-2">
                    <span className="text-slate-700 font-semibold">Total to Deduct</span>
                    <span className="font-bold text-[#e65a28] text-xl">Rs. {(Number(amount) + 50).toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
                  </div>
                </div>

                <div className="flex items-center justify-center gap-4">
                  <button onClick={() => setStep('form')} className="rounded-xl border border-slate-200 bg-white px-8 py-3.5 font-bold text-slate-700 transition-all hover:bg-slate-50">
                    Back to Edit
                  </button>
                  <button onClick={handleTransfer} disabled={loading} className="rounded-xl bg-[#e65a28] px-8 py-3.5 font-bold text-white shadow-md shadow-orange-500/20 transition-all hover:bg-[#d44d1e] hover:shadow-lg disabled:opacity-70 disabled:cursor-not-allowed min-w-[160px]">
            ...
            {step === 'success' && (
              <div className="relative z-10 text-center py-12">
                ...
                <h3 className="text-3xl font-bold text-slate-900 mb-2">Transfer Successful</h3>
                <p className="text-slate-500 mb-8">Ref: {confirmation}</p>
                <button onClick={() => {
                  setAmount(''); setAccountNumber(''); setAccountName(''); setBank(''); setDescription(''); setErrors({}); setConfirmation(null); setStep('form');
            ...
            {step === 'failure' && (
              <div className="relative z-10 text-center py-12">
                ...
                <h3 className="text-3xl font-bold text-slate-900 mb-4">Transfer Failed</h3>
                <p className="text-slate-500 mb-8 max-w-md mx-auto whitespace-pre-line">{failReason}</p>
                <button onClick={() => setStep('form')} className="rounded-xl border border-slate-200 bg-white px-8 py-3.5 font-bold text-slate-700 transition-all hover:bg-slate-50 shadow-sm">
                  Try Again
                </button>
              </div>
            )}
