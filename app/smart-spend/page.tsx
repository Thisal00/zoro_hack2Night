  ...
  data.transactions.forEach((tx) => {
    if (tx.isExpense) {
      categorySpend[tx.category] =
        (categorySpend[tx.category] || 0) + Number(tx.amount)
    }
  })
        ...
        <header className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Analyze and manage your expenses.
            </p>
          </div>
                  ...
                  <div className="p-2 space-y-1">
                    <div className="p-3 rounded-xl hover:bg-slate-50 transition-colors cursor-pointer">
                      ...
                      <p className="text-xs text-slate-500 mt-1">
                        You've spent 50% of your transport budget.
                      </p>
                    </div>
                  </div>
                </div>
        ...
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="rounded-3xl bg-white border border-slate-200 p-6 shadow-sm relative overflow-hidden">
              ...
              {loading ? (
                ...
                <p className="text-3xl font-black text-slate-900">
                  Rs. {data.budgetRemaining.toLocaleString()}
                </p>
              )}
              <div className="mt-4 h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                <div
                  className="h-full bg-[#e65a28] rounded-full"
                  style={{
                    width: `${Math.min(100, (data.totalSpent / (data.totalSpent + data.budgetRemaining)) * 100)}%`
                  }}
                />
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="rounded-3xl bg-white border border-slate-200 p-8 shadow-sm">
              ...
              ) : data.transactions.filter((t) => t.isExpense).length === 0 ? (
                <p className="text-slate-500 text-center py-4">
                  No recent spend.
                </p>
              ) : (
                <div className="space-y-4">
                  {data.transactions
                    ...
                    .map((tx, idx) => (
                      <div
                        key={idx}
                        className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 hover:bg-orange-50 transition-colors border border-transparent hover:border-orange-100"
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-xs font-medium text-[#e65a28] mt-0.5">
                              {tx.category}
                            </p>
                          </div>
                        </div>
