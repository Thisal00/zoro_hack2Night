...
export default function AccountsPage() {
  ...
  const validateField = (name: string, value: string) => {
    ...
    switch (name) {
      case 'accountNumber':
        ...
        else if (value.length < 8 || value.length > 20) error = 'Must be between 8 and 20 digits'
        break
      case 'accountName':
  ...
  const goToList = () => { resetForm(); setScreen('list'); router.push('/bank-accounts') }
  const goToAdd = () => { resetForm(); setScreen('add'); router.push('/bank-accounts?mode=add') }
  const goToEdit = () => { setScreen('edit'); router.push('/bank-accounts?mode=edit') }
  ...
  const handleUpdateAccount = (e: React.FormEvent) => {
    e.preventDefault()
    if (!formData.accountNumber.trim()) { alert('Please enter an account number first'); return }
    router.push(`/bank-accounts?mode=edit&accountNumber=${formData.accountNumber}&accountName=${formData.accountName || ''}&email=${formData.email || ''}&nickname=${formData.nickname || ''}`)
  }

  const handleEditNickname = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!nickname.trim() || nickname.trim().length < 2) { alert('Valid nickname is required'); return }
    try {
      const userId = getCookie('user_id') || '1'
  ...
  const handleDeleteAccount = async (accountNumber: string) => {
    ...
    try {
      const userId = getCookie('user_id') || '1'
      const res = await fetch(`/api/accounts/delete?userId=${userId}&accountNumber=${accountNumber}`, {
        method: 'DELETE'
      })
      const data = await res.json()
      if (res.ok && data.ok) {
      ...
      <main className="flex-1 overflow-y-auto px-6 py-8 md:px-10 lg:px-12">
        <header className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Manage your linked bank accounts and balances.</p>
          </div>
                ...
                <div className="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden z-50">
                  ...
                  <div className="p-2 space-y-1">
                    <div className="p-3 rounded-xl hover:bg-slate-50 transition-colors cursor-pointer">
                      <p className="text-sm font-medium text-slate-900">Account Verified</p>
                      <p className="text-xs text-slate-500 mt-1">Your new account has been verified.</p>
                    </div>
                  </div>
                </div>
              )}
        ...
        <div className="max-w-5xl mx-auto">
          {screen === 'list' && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {loading ? (
                <div className="col-span-full flex items-center justify-center py-20 text-slate-400">Loading accounts...</div>
              ) : accounts.length === 0 ? (
                <div className="col-span-full flex items-center justify-center py-20 text-slate-400">No accounts found.</div>
              ) : (
                accounts.map((acc, idx) => (
                  <div key={idx} className="group relative overflow-hidden rounded-3xl bg-white border border-slate-200 p-6 shadow-sm transition-all hover:-translate-y-1 hover:border-[#e65a28] hover:shadow-md">
                    ...
                    <div className="flex justify-between items-start mb-6">
                      <div className="size-12 rounded-full bg-slate-50 flex items-center justify-center p-2 border border-slate-100">
                        <img src="/account-logo.png" alt="bank logo" className="size-full object-contain" />
                      </div>
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-slate-900 mb-1 truncate">{acc.account_name}</h3>
                      <p className="text-sm font-mono text-slate-500 mb-6 tracking-widest">{acc.account_number}</p>
                      
                      <div className="pt-4 border-t border-slate-100 flex justify-between items-end">
                        <span className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Available Balance</span>
                        <span className="text-lg font-bold text-[#e65a28]">Rs. {Number(acc.balance).toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                ))
              ...
              >
                ...
                <h3 className="text-lg font-semibold text-slate-500 group-hover:text-[#e65a28] transition-colors">Add Bank Account</h3>
              </button>
            </div>
          )}
          ...
          {screen === 'add' && (
            <div className="max-w-2xl mx-auto">
              <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm relative overflow-hidden">
                ...
                <h2 className="text-2xl font-bold text-slate-900 mb-8">Add Another Bank Account</h2>
                ...
                <form className="relative z-10 space-y-5">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div className="col-span-full md:col-span-1">
                      <label className="block text-sm font-medium text-slate-700 mb-1.5">Bank Account Name</label>
                      <input
                        type="text"
                        name="accountName"
                    ...
                    <div className="col-span-full">
                      <label className="block text-sm font-medium text-slate-700 mb-1.5">Nickname</label>
                      <input
                        type="text"
                        name="nickname"
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 mt-8 pt-6 border-t border-slate-100">
                    <button type="button" onClick={goToList} className="w-full sm:w-auto rounded-xl border border-slate-200 bg-white px-8 py-3.5 font-bold text-slate-700 shadow-sm transition-all hover:bg-slate-50 hover:text-slate-900">
                      Cancel
                    </button>
                    <button type="button" onClick={handleAddAccount} className="w-full sm:w-auto rounded-xl bg-[#e65a28] px-8 py-3.5 font-bold text-white shadow-md shadow-orange-500/20 transition-all hover:bg-[#d44d1e] hover:shadow-lg hover:shadow-orange-500/30">
                      Add Account
                    </button>
                    <button type="button" onClick={handleUpdateAccount} className="w-full sm:w-auto rounded-xl border border-slate-200 bg-slate-50 px-8 py-3.5 font-bold text-slate-700 transition-all hover:bg-slate-100">
                      Update Existing
                    </button>
                  </div>
                </form>
          ...
          {screen === 'edit' && (
            <div className="max-w-xl mx-auto">
              <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm relative overflow-hidden">
                ...
                <h2 className="text-2xl font-bold text-slate-900 mb-8">Edit Nickname</h2>

                <form onSubmit={handleEditNickname} className="relative z-10 space-y-6">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1.5">Bank Account Number</label>
                    <input
                      type="text"
                  ...
                  <div className="flex gap-4 mt-8 pt-6 border-t border-slate-100">
                    <button type="button" onClick={goToList} className="flex-1 rounded-xl border border-slate-200 bg-white px-8 py-3.5 font-bold text-slate-700 shadow-sm transition-all hover:bg-slate-50">
                      Cancel
                    </button>
                    <button type="submit" className="flex-1 rounded-xl bg-[#e65a28] px-8 py-3.5 font-bold text-white shadow-md shadow-orange-500/20 transition-all hover:bg-[#d44d1e] hover:shadow-lg hover:shadow-orange-500/30">
                      Save Changes
                    </button>
                  </div>
                </form>
